...
L.divIcon({
    ...
    this.geoJsonDataHome.forEach(geoJsonItem => {
      // const geoJsonPositionLatLng = L.latLng(geoJsonItem.averageLatitude, geoJsonItem.averageLongitude)
      // distance = this.coordinateCenterBogota.distanceTo(geoJsonPositionLatLng);
      // if(distance <= 19290 ){
      // midPointLat += geoJsonItem.averageLatitude
      // midPointLng += geoJsonItem.averageLongitude
      // count += 1
      // console.log(' calculado :', count, midPointLat, midPointLng, geoJsonItem.averageLatitude, geoJsonItem.averageLongitude);
      // }
      totalGlobal += geoJsonItem.totalFeatures
    })
    // midPointLat /= this.geoJsonDataHome.length > 0 ? this.geoJsonDataHome.length : 1
    // midPointLng /= this.geoJsonDataHome.length > 0 ? this.geoJsonDataHome.length : 1
    // let midPoint = { lat: midPointLat, lng: midPointLng };
    // console.log('Punto medio calculado :', midPoint, midPointLat, midPointLng, this.geoJsonDataHome.length);
    ...
    // let midPointLatLng: L.LatLng = L.latLng([midPointLat, midPointLng])
    ...
    //    this.mapHome.fitBounds(bounds);
    // this.mapHome.flyTo(midPoint, this.mapHome.getZoom());
    let zoom = 12
